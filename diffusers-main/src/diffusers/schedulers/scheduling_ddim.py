...
class DDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.alphas_cumprod = torch.cumprod(self.alphas, dim=0)   # 连乘
    ...
    def step(
        ...
        prev_timestep = timestep - self.config.num_train_timesteps // self.num_inference_steps   # 上一个时间步
        ...
        # 定义模型预测输出：预测噪声、预测样本、预测v(flow matching)
        if self.config.prediction_type == "epsilon":
            pred_original_sample = (sample - beta_prod_t ** (0.5) * model_output) / alpha_prod_t ** (0.5)
            pred_epsilon = model_output
        elif self.config.prediction_type == "sample":   # 目前走这里，预测轨迹
        ...
        variance = self._get_variance(timestep, prev_timestep)   # 计算方差
        std_dev_t = eta * variance ** (0.5)   # 计算噪声标准差
        ...
        prev_sample = alpha_prod_t_prev ** (0.5) * pred_original_sample + pred_sample_direction     # 这里是引用了 DDIM 即论文中的公式
    ...
    def add_noise(
        ...
        return noisy_samples     #利用核心公式进行加噪
